...
- Config writes are atomic, failed imports roll back in-memory changes, invalid
  Unicode is rejected before mutation, and export works on Windows without the
  Unix-only `fchmod` API.
