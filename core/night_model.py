...
# Compatibility boards retained from the feature-complete v2.2.x app.  The
# Pro viewer consumes richer stat-table metadata, but Simple/Classic callers
# and their regression fixtures still rely on these eight top-ten lists.
HIGHLIGHT_TOP_N = 10
_HIGHLIGHTS_SPEC = (
...
def _norm_highlight_header(cell):
    """Normalize a source header for exact highlight metric matching."""
    match = _HL_IMG_ALT_RE.search(cell)
    name = match.group(1) if match else cell
    return re.sub(r"[^A-Za-z0-9]", "", name).lower()


def _highlight_metric_col(header_cells, token):
    for index, cell in enumerate(header_cells):
        if _norm_highlight_header(cell) == token:
            return index
    return None


def _parse_highlight_rows(line_iter, value_col):
    """Parse one exact metric column without weakening the richer Pro parser."""
    rows = []
    for line in line_iter:
        if not _is_data_row(line):
            continue
        raw_cells = _cells(line)
        match = _TOOLTIP_RE.search(line)
        account = match.group(1).strip() if match else None
        profession = _prof(line)
        cells = [_plain(cell) for cell in raw_cells]
        rank = _num(raw_cells[1]) if len(raw_cells) > 1 else None
        name = None
        for index, cell in enumerate(cells):
            if index == 0 and rank is not None:
                continue
            raw = raw_cells[index] if index < len(raw_cells) else ""
            if cell and not _NUM_RE.match(raw) and re.search(r"[A-Za-z]", cell):
                name = cell
                break
        if name is None or value_col >= len(raw_cells):
            continue
...
def _parse_highlights(tiddlers):
    """Build the eight v2.2.x headline boards from their source columns."""
    boards = {}
    warnings = []
    ...
    for key, suffix, token in _HIGHLIGHTS_SPEC:
        ...
        if tag:
            source = next((
                item for item in tiddlers
                if item.get("title", "") == tag + suffix
            ), None)
        if not source or not source.get("text"):
            boards[key] = []
            warnings.append(
                f"highlights.{key}: source {suffix.lstrip('-')} tiddler missing"
            )
            continue
        lines = source["text"].split("\n")
        header = next((line for line in lines if _is_header_row(line)), None)
        column = (
            _highlight_metric_col(_cells(header), token) if header else None
        )
        if column is None:
            boards[key] = []
            warnings.append(
            ...
            continue
        rows = _parse_highlight_rows(lines, column)
        rows = [
            row for row in rows
            if not re.search(r"\b(average|totals)\b", row["name"], re.IGNORECASE)
        ]
        best = {}
        for row in rows:
            player_key = row["account"] or row["name"]
            previous = best.get(player_key)
            if previous is None or row["value"] > previous["value"]:
                best[player_key] = row
        rows = sorted(best.values(), key=lambda row: row["value"], reverse=True)
        boards[key] = rows[:HIGHLIGHT_TOP_N]
        for rank, row in enumerate(boards[key], 1):
            row["rank"] = rank
...
def build_night_model(
    ...
    try:
        boards, highlight_warnings = _parse_highlights(copy.deepcopy(tiddlers))
        model["highlights"] = boards
        warnings.extend(highlight_warnings)
    except Exception as exc:  # noqa: preserve the rest of the night model
        warnings.append(f"highlights: {type(exc).__name__}: {exc}")
        model["highlights"] = {
            key: [] for key, _, _ in _HIGHLIGHTS_SPEC
        }
